...
const ThemeModeSwitcher = ({ toggleBtnClass = "", toggleBtnIconClass = "fs-1", menuPlacement = "bottom-end", menuTrigger = "{default: 'click', lg: 'hover'}" }: Props) => {
  const { setLayoutType } = useLayout();
  const { mode, menuMode, updateMode, updateMenuMode } = useThemeMode();
  const calculatedMode = mode === "system" ? systemMode : mode;
  const switchMode = (_mode: ThemeModeType) => {
    const mode = _mode !== "system" ? _mode : systemMode;
    setLayoutType(`${mode}-sidebar`);
