...
interface IGroupsChannelListPaginationProps {
  filterGroupChannel: any;
  setFilterGroupChannel: Dispatch<
    SetStateAction<{
      limit: number;
      offset: number;
      name: string;
...
const GroupChannelListPagination = ({ filterGroupChannel, setFilterGroupChannel }: IGroupsChannelListPaginationProps) => {
  const params = useParams();
  const id = params.id as string;
  const [pagination, setPagination] = useState<PaginationState>({
    page: 1,
    items_per_page: filterGroupChannel.limit,
    links: [],
  });
  ...
  useEffect(() => {
    if (groupChannelListQuery.data) {
      const noOfLinks = groupChannelListQuery.data.total;
      const noOfPages = Math.ceil(noOfLinks / groupChannelListQuery.data.limit);
      const links = [];
  ...
  }, [groupChannelListQuery.data]);

  const onChangePageSize = (e: any) => {
    setFilterGroupChannel((prevState: any) => ({
      ...prevState,
    ...
    <div className="row">
      <div className="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
        ...
        <div id="kt_table_groups_info" className="dataTables_info">
          {isLoading ? "Loading..." : `Total ${groupChannelListQuery.data?.total || 0} assets`}
        </div>
      </div>
...
export { GroupChannelListPagination };
