...
interface IMembersListPaginationProps {
  filterMember: any;
...
const MembersListPagination = ({ filterMember, setFilterMember }: IMembersListPaginationProps) => {
  const params = useParams();
  const domainId = params.id as string;
  const [pagination, setPagination] = useState<PaginationState>({
    page: 1,
    items_per_page: filterMember.limit,
    links: [],
  });
  const memberListQuery = useQuery({
    queryKey: [`memberList`, domainId, filterMember],
    queryFn: async () => getMemberList(domainId, filterMember).catch((error) => toast.error(error.message)),
    enabled: false,
  });
  ...
  useEffect(() => {
    if (memberListQuery.data) {
      const noOfLinks = memberListQuery.data.total;
      const noOfPages = Math.ceil(noOfLinks / memberListQuery.data.limit);
      const links = [];
  ...
  return (
    <div className="row">
      <div className="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
        ...
        <div id="kt_table_members_info" className="dataTables_info">
          {isLoading ? "Loading..." : `Total ${memberListQuery.data?.total || 0} members`}
        </div>
      </div>
      <div className="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
        <div id="kt_table_members_paginate">
