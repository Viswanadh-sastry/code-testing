...
interface IInvitationsListPaginationProps {
  filterInvitation: any;
...
const InvitationsListPagination = ({ filterInvitation, setFilterInvitation }: IInvitationsListPaginationProps) => {
  const [pagination, setPagination] = useState<PaginationState>({
    page: 1,
    items_per_page: filterInvitation.limit,
    links: [],
  });
  const invitationListQuery = useQuery({
    queryKey: [`invitationList`, filterInvitation],
    queryFn: async () => getInvitationList(filterInvitation).catch((error) => toast.error(error.message)),
    enabled: false,
  });
  const isLoading = invitationListQuery.isLoading;

  useEffect(() => {
    if (invitationListQuery.data) {
      const noOfLinks = invitationListQuery.data.total;
      const noOfPages = Math.ceil(noOfLinks / invitationListQuery.data.limit);
      const links = [];
  ...
  return (
    <div className="row">
      <div className="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start">
        ...
        <div id="kt_table_invitations_info" className="dataTables_info">
          {isLoading ? "Loading..." : `Total ${invitationListQuery.data?.total || 0} invitations`}
        </div>
      </div>
      <div className="col-sm-12 col-md-7 d-flex align-items-center justify-content-center justify-content-md-end">
        <div id="kt_table_invitations_paginate">
...
export { InvitationsListPagination };
