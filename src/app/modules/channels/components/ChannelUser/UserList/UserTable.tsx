...
import { getChannelUserList } from "../../../api/ChannelUserAPI";
...
const UserTable = () => {
  ...
  const userListQuery = useQuery({
    ...
    queryFn: async () =>
      getChannelUserList(id, filterUser)
        .then(async (response) => {
  ...
  return (
    <>
      <KTCard>
        <UserListHeader onShowAddUser={onShowAddUser} setFilterUser={setFilterUser} tab={tab} setTab={setTab} />
            ...
            <table id="kt_table_groups" className="table align-middle table-row-dashed fs-6 dataTable no-footer" {...getTableProps()}>
              <thead>
                <tr className="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                  {headers.map((column: ColumnInstance<ChannelUser>) => (
              ...
              <tbody className="text-gray-600" {...getTableBodyProps()}>
                {rows.length > 0 ? (
                  rows.map((row: Row<ChannelUser>, i) => {
                    prepareRow(row);
