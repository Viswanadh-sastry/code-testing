...
const AssetListPagination = ({ assetHistoryListQuery, currentPage, setCurrentPage, itemsPerPage, setItemsPerPage }: IGroupListPaginationProps) => {
  const totalItems = assetHistoryListQuery.data?.length || 0;
  const totalPages = Math.ceil(totalItems / itemsPerPage);

  const handlePageChange = (page: number) => {
    if (page < 1 || page > totalPages || assetHistoryListQuery.isLoading) return;
    setCurrentPage(page);
  };
    ...
    <div className="row">
      <div className="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start py-7">
        ...
        <div id="kt_table_channels_info" className="dataTables_info">
          {assetHistoryListQuery.isLoading ? "Loading..." : `Total ${totalItems} histories`}
        </div>
      </div>
